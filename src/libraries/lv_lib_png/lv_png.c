...
#define LV_FS_PC_PATH   "/fs"
#endif
            char buf[256];
            sprintf(buf, LV_FS_PC_PATH "%s", &fn[2]);
...
static lv_res_t decoder_open(lv_img_decoder_t *decoder, lv_img_decoder_dsc_t *dsc)
{
    ...
    if (dsc->src_type == LV_IMG_SRC_FILE) {
        ...
        if (!strcmp(&fn[strlen(fn) - 3], "png")) {             /*Check the extension*/
            ...
            error = lodepng_load_file(&png_data, &png_data_size, buf);   /*Load the file*/
            if (error) {
            ...
            uint16_t png_width;             /*Will be the width of the decoded image*/
            uint16_t png_height;            /*Will be the width of the decoded image*/
    ...
    /*If it's a PNG file in a  C array...*/
        ...
        uint16_t png_width;             /*No used, just required by he decoder*/
        uint16_t png_height;            /*No used, just required by he decoder*/
